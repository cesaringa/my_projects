...
PImage sobelxy_img1;
PImage sobelxy_img2;
PImage threshold_img1;
PImage threshold_img2;
PImage grayscale_img1;
PImage grayscale_img2;
int number_vectors = 8;              // Number of elements to represent the oriented gradients
int gridx = 2;
int gridy = 3;
void setup() { 
  size(1000, 1100);
  img1 = loadImage("7-31.jpg");       // Load the original image
  img1.loadPixels();

  img2 = loadImage("2t.jpg");       // Load the original image
  img2.loadPixels();

  grayscale_img1 = grayscale(img1);
  grayscale_img2 = grayscale(img2);

  threshold_img1 = adaptive_threshold(grayscale_img1);
  threshold_img2 = adaptive_threshold(grayscale_img2);

  sobelxy_img1=sobelxy(threshold_img1);
  sobelxy_img2=sobelxy(threshold_img2);

  float [][] new_matrix_1 = feature_vector(threshold_img1);
  float [][] new_matrix_2 = feature_vector(threshold_img2);

  float c = 0.0;
  c = compare(new_matrix_1, new_matrix_2);

  image(img1, 0, 0);                 // Displays the image from point (0,0)
  image(grayscale_img1, 0, 260);
  image(threshold_img1, 0, 520);
  image(sobelxy_img1, 0, 780);

  image(img2, 200, 0);                 // Displays the image from point (0,0)
  image(grayscale_img2, 200, 260);
  image(threshold_img2, 200, 520);
  image(sobelxy_img2, 200, 780);

  println("Similarity is =", c );
}


PImage histogram(PImage img) {
  ...
  stroke(0);
  for (int i = 0; i < 256; i++)
    line(i, height - 1, i, (height - 1) * (1 - (float) h[i] / hMax));
  temp.updatePixels();
  return temp;
}
...
PImage sobelxy(PImage img) {
  ...
  for (int y = 1; y < img.height-1; y++)  // Skip top and bottom edges
    for (int x = 1; x < img.width-1; x++) { // Skip left and right edges
      ...
      temp.pixels[y*img.width + x] = color(sqrt(gx*gx + gy*gy));
    }
